...
// DeepSeek account balance (RMB)
function getDeepSeekBalance() {
  // Try env first, then fallback to direct query with managed key
  let apiKey = process.env.ANTHROPIC_AUTH_TOKEN || '';
  if (!apiKey || apiKey === 'PROXY_MANAGED') {
    apiKey = 'sk-48b219c8230b4b96bd7a06c875a5f3db';
  }
  if (!apiKey) return null;
  try {
    const url = 'https://api.deepseek.com/user/balance';
    const res = require('child_process').execSync(
      'curl -s --connect-timeout 3 --max-time 5 "' + url + '" -H "Authorization: Bearer ' + apiKey + '"',
...
// Model pricing constants (RMB per million tokens)
const MODEL_PRICING = {
  'deepseek-v4-flash': { output: 2, input: 1, label: 'Flash' },
  'deepseek-v4-pro': { output: 6, input: 3, label: 'Pro' },
};
const DEFAULT_PRICING = { output: 2, input: 1, label: 'Flash' };

// Get current model pricing
function getModelPricing() {
  const model = (process.env.ANTHROPIC_MODEL || '').toLowerCase();
  for (const [key, val] of Object.entries(MODEL_PRICING)) {
    if (model.includes(key.replace('deepseek-v4-', ''))) return val;
  }
  return DEFAULT_PRICING;
}

// Estimate remaining conversations based on balance and model pricing
function estimateRemainingConversations(balance) {
  if (!balance || balance <= 0) return null;
  const pricing = getModelPricing();
  // Assume ~500K output tokens per conversation (conservative)
  const tokensPerConversation = 500000;
  const costPerConversation = (pricing.output * tokensPerConversation) / 1000000;
  const remaining = Math.floor(balance / costPerConversation);
...
function generateStatusline() {
  ...
  if (dsBalance && dsBalance.balance > 0) {
    const balColor = dsBalance.balance >= 10 ? c.brightGreen : dsBalance.balance >= 1 ? c.brightYellow : c.brightRed;
    header += '  ' + c.dim + '\u2502' + c.reset + '  ' + balColor + '\u00A5' + dsBalance.balance.toFixed(2) + c.reset;
  }
  lines.push(header);
